...
class GRPOScriptArguments(ScriptArguments):
    ...
    custom_reward_weights: str = field(
        default=None,
        metadata={"help": "Comma-separated weights for each reward function (e.g., '0.1,0.45,0.45'). Must match the number of reward functions."},
    )
    # Note: max_completion_length and max_prompt_length are already defined in parent class ScriptArguments, no need to repeat
...
def reasoning_similarity_reward(completions, solution, **kwargs):
    """
    Use DeepSeek-V3 via OpenAI SDK to score generated vs. reference reasoning (0–1).
    """
    client = OpenAI(
    ...
    for comp, sol in zip(completions, solution):
        gen_match = re.search(r"<Reasoning>(.*?)</Reasoning>", comp[0]["content"], re.DOTALL)
        ref_match = re.search(r"<Reasoning>(.*?)</Reasoning>", sol, re.DOTALL)
        gen_text = gen_match.group(1).strip() if gen_match else ""
        ref_text = ref_match.group(1).strip() if ref_match else ""

        prompt = (
            "Compare these two reasoning passages and return a similarity score 0–1.\n"
            f"Generated:\n{gen_text}\n\nReference:\n{ref_text}"
        )
        resp = client.chat.completions.create(
            model="deepseek-chat",
            messages=[
                {"role": "system", "content": "You are an AI assistant that evaluates the similarity between two reasoning processes. Your response MUST follow the format 'Similarity score: <score>', where <score> is a single floating-point number between 0.0 and 1.0 representing the similarity score."},
                {"role": "user",   "content": prompt}
            ],
            stream=False
        )
        # parse numeric score from the assistant’s reply
        # score = float(resp.choices[0].message.content.strip())
        content = resp.choices[0].message.content.strip()
        # score_match = re.search(r'Similarity score:\s*([\d.]+)', content)
        score_match = re.search(r'[Ss]imilarity [Ss]core:\s*([\d.]+)', content)
        if score_match:
            score = float(score_match.group(1))
        else:
            # If score not found, set default value
            score = 0.5  # Default medium similarity
            print(f"Warning: Could not extract similarity score from: {content}")
        rewards.append(score)
    return rewards


def answer_similarity_reward(completions, solution, **kwargs):
    """
    Use DeepSeek-V3 via OpenAI SDK to score generated vs. reference answer (0–1).
    """
    client = OpenAI(
        api_key=os.getenv("DEEPSEEK_API_KEY"),
        base_url="https://api.deepseek.com"
    ...
    rewards = []
    for comp, sol in zip(completions, solution):
        gen_match = re.search(r"<Answer>(.*?)</Answer>", comp[0]["content"], re.DOTALL)
        ref_match = re.search(r"<Answer>(.*?)</Answer>", sol, re.DOTALL)
        gen_text = gen_match.group(1).strip() if gen_match else ""
        ref_text = ref_match.group(1).strip() if ref_match else ""

        prompt = (
            "Compare these two answer texts and return a similarity score 0–1.\n"
            f"Generated:\n{gen_text}\n\nReference:\n{ref_text}"
        )
        resp = client.chat.completions.create(
            model="deepseek-chat",
            messages=[
                {"role": "system", "content": "You are an AI assistant that evaluates the similarity between two answer. Your response MUST follow the format 'Similarity score: <score>', where <score> is a single floating-point number between 0.0 and 1.0 representing the similarity score."},
                {"role": "user",   "content": prompt}
            ],
            stream=False
        )
        ...
        if score_match:
            ...
            # If score not found, set default value
            score = 0.6  # Default medium similarity
            print(f"Warning: Could not extract similarity score from: {content}")
        rewards.append(score)
    return rewards


def format_and_tag_reward(completions, **kwargs):
    """
    Ensure both <Reasoning>…</Reasoning> and <Answer>…</Answer> tags
    appear in the output. 1.0 if both found, 0.0 otherwise.
    """
    pattern = r"<Reasoning>.*?</Reasoning>.*<Answer>.*?</Answer>"
    rewards = []
    for comp in completions:
        ok = bool(re.search(pattern, comp[0]["content"], re.DOTALL))
...
reward_funcs_registry = {
    ...
    "format_tagged": format_and_tag_reward,
    "reasoning_sim": reasoning_similarity_reward,
    "answer_sim": answer_similarity_reward,
}
...
def main(script_args, training_args, model_args):
    ...
    # question_prompt will be generated dynamically in make_conversation_from_jsonl based on question_type
    ...
    custom_reward_weights = None
    if script_args.custom_reward_weights:
        custom_reward_weights = [float(w) for w in script_args.custom_reward_weights.split(',')]
        if len(custom_reward_weights) != len(reward_funcs):
            raise ValueError(f"Number of reward weights ({len(custom_reward_weights)}) must match number of reward functions ({len(reward_funcs)})")
        print("custom_reward_weights:", custom_reward_weights)
    ...
    # Detect if user is providing separate train/validation files
    # If 2 files provided, treat first as train, second as validation
    separate_val_file = len(data_files) == 2
    
    # Helper function to load data from a single file
    def load_single_file(data_file, image_folder, accu_reward_method):
        file_data = []
        with open(data_file, 'r') as f:
            for line in f:
                ...
                # Handle both 'value' and 'content' keys in conversations
                conv_key = 'value' if 'value' in item['conversations'][0] else 'content'
                item['problem'] = item['conversations'][0][conv_key].replace('<image>', '')
                
                # Handle solution that could be a float or string
                solution_value = item['conversations'][1][conv_key]
                ...
                item['question_type'] = item.get('question_type', 'open_ended')  # Preserve question_type for dynamic prompt
    ...
    # Load training data (always the first file)
    if separate_val_file:
        print(f"Detected separate train/validation files: {data_files[0]} and {data_files[1]}")
        train_data = load_single_file(data_files[0], image_folders[0], accu_reward_methods[0])
        val_data = load_single_file(data_files[1], image_folders[1], accu_reward_methods[1] if len(accu_reward_methods) > 1 else accu_reward_methods[0])
        print(f"Loaded {len(train_data)} training samples and {len(val_data)} validation samples")
        dataset = Dataset.from_list(train_data)
        val_dataset = Dataset.from_list(val_data)
    else:
        # Original behavior: merge all files
        all_data = []
        for data_file, image_folder, accu_reward_method in zip(data_files, image_folders, accu_reward_methods):
            all_data.extend(load_single_file(data_file, image_folder, accu_reward_method))
        dataset = Dataset.from_list(all_data)
        val_dataset = None

    def make_conversation_from_jsonl(example):
        # Dynamically generate prompt based on question_type
        question_type = example.get('question_type', 'open_ended')
        dynamic_prompt = vlm_module_cls.get_question_template(
        ...
        if 'image_path' in example and example['image_path'] is not None:
            ...
            return {
                ...
                'question_type': question_type,  # Pass through for debugging
                'prompt': [{
                    'role': 'user',
                    'content': [
                        *({'type': 'image', 'text': None} for _ in range(len(example['image_path']))),
                        {'type': 'text', 'text': dynamic_prompt.format(Question=example['problem'])}
                    ]
                }]
            }
        else:
    ...
    # Split dataset for validation if not already provided via separate file
    splits = {'train': dataset}
    if val_dataset is not None:
        # Use the separately loaded validation dataset
        splits['validation'] = val_dataset
        print(f"Using separate validation dataset with {len(val_dataset)} samples")
    elif script_args.val_split_ratio > 0:
        ...
        print(f"Split training data into {len(splits['train'])} train and {len(splits['validation'])} validation samples")
