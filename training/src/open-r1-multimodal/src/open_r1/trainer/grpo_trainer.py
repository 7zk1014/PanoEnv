...
class VLMGRPOTrainer(Trainer):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if peft_config is not None:
            print("Applying LoRA...")
            print(f"Vision module keywords to exclude: {self.vision_modules_keywords}")
            def find_all_linear_names(model, multimodal_keywords):
                ...
                for m in list(lora_module_names):  # needed for 16-bit, use list() to avoid set change during iteration
            ...
            if len(target_modules) > 0:
                print(f"Sample target modules: {target_modules[:5]}")
            peft_config.target_modules = target_modules
            
            # Debug: Check peft_config before applying
            print(f"🔧 peft_config type: {type(peft_config)}")
            print(f"🔧 peft_config: {peft_config}")
            
            model = get_peft_model(model, peft_config)
            
            # Debug: Check if model is actually a PeftModel
            print(f"🔧 model type after get_peft_model: {type(model)}")
            print(f"🔧 Is PEFT model: {is_peft_model(model)}")
            
            # Print LoRA adapter parameters
            if hasattr(model, 'print_trainable_parameters'):
                model.print_trainable_parameters()
            else:
                print("⚠️  WARNING: model.print_trainable_parameters() not found! PEFT may not be applied.")

        # Freeze vision modules (must be done AFTER LoRA to avoid unfreezing base model)
        ...
        # Debug: print all trainable parameter names to verify LoRA is working
        if peft_config is not None:
            ...
            if lora_params == 0:
                print("  ⚠️ WARNING: No LoRA parameters found! LoRA may not be applied correctly.")
        ...
        if reward_weights is not None:
            if len(reward_weights) != len(reward_funcs):
                raise ValueError(f"Number of reward weights ({len(reward_weights)}) must match number of reward functions ({len(reward_funcs)})")
            ...
            print(f"Using reward weights: {self.reward_weights.tolist()}")
        ...
        # Force set max_completion_length
        self.max_completion_length = args.max_completion_length  # = |o_i| in the GRPO paper
        if self.max_completion_length is None:
            print("⚠️  WARNING: max_completion_length is None! Setting default to 64")
            self.max_completion_length = 64
        else:
            print(f"✅ max_completion_length set to: {self.max_completion_length}")
        ...
        print(f"✅ GenerationConfig.max_new_tokens: {self.generation_config.max_new_tokens}")
    ...
    def _generate_and_score_completions(self, inputs: dict[str, Union[torch.Tensor, Any]], model) -> dict[str, Union[torch.Tensor, Any]]:
        ...
        rewards = torch.matmul(rewards_per_func, self.reward_weights.to(rewards_per_func.device))
        ...
        scale_factor = 5.0 
        advantages = scale_factor * (rewards - mean_grouped_rewards) / (std_grouped_rewards + 1e-4)
        ...
        # After computing reward_per_func, add this:
        weighted_reward_per_func = reward_per_func * self.reward_weights.to(reward_per_func.device)
        for i, reward_func in enumerate(self.reward_funcs):
            if isinstance(reward_func, PreTrainedModel):
                reward_func_name = reward_func.config._name_or_path.split("/")[-1]
            else:
                reward_func_name = reward_func.__name__
            self._metrics[f"rewards/{reward_func_name}"].append(reward_per_func[i].item())
        self._metrics[f"weighted_rewards/{reward_func_name}"].append(weighted_reward_per_func[i].item())
    ...
    def log(self, logs: dict[str, float], start_time: Optional[float] = None) -> None:
        ...
        if "loss" in logs and abs(logs["loss"]) < 1e-4:
            logs["loss"] = f"{logs['loss']:.2e}"  # Use scientific notation for small values
        logs = {**logs, **metrics}
